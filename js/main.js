...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Dynamically position floating quick actions under header on mobile
    function positionQuickActions(){
        const fqa = document.querySelector('.floating-quick-actions');
        const headerEl = document.getElementById('header');
        if (!fqa || !headerEl) return;
        if (window.innerWidth <= 768) {
            const h = headerEl.getBoundingClientRect().height;
            fqa.style.top = `${Math.ceil(h + 10)}px`;
        } else {
            fqa.style.top = '100px';
        }
    }
